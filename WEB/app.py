...
class LegalReport(FPDF):
    def header(self):
        ...
        # Официальный заголовок в PDF-отчете
...
# --- ЦЕПОЧКА 4-4-4 BURAN (ИНТЕЛЛЕКТУАЛЬНЫЙ КОНТУР) [cite: 2026-01-20] ---
def run_legal_factory(user_task, full_context):
    ...
    jur = client.chat.completions.create(
        model="deepseek-chat",
        messages=[{"role": "system", "content": "Этап 3: Ты Юрист. Обоснуй позицию по закону (например, Unfair Dismissals Act 1977)."},
    ...
    con = client.chat.completions.create(
        model="deepseek-chat",
        messages=[{"role": "system", "content": "Этап 4: Ты Контролер. Проверь логику и отсутствие человеческого фактора в решении."},
...
# Изменяем название во вкладке браузера
...
if st.button("👑 ПОЛУЧИТЬ ЮРИДИЧЕСКИЙ ВЕРДИКТ"):
    if not user_instruction:
        ...
        with st.spinner("Интеллектуальный контур проводит верификацию..."):
            ...
            # Запуск конвейера цифровых двойников (скрытая мощь системы) [cite: 2026-01-05]
            ana, bru, jur, con, judge = run_legal_factory(user_instruction, combined_text)
            
            # Визуализация работы контуров
            st.markdown("### 🧬 Протокол обработки данных")
            col1, col2 = st.columns(2)
            with col1:
                st.error(f"**Контур анализа рисков (Оппонент):**\n{bru[:400]}...")
            with col2:
